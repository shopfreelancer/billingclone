...
class CustomerTicketsController extends AppController {
	...
	// Join liess sich nicht richtig über das Model und schon gar nicht Paginate abbilden.
		...
		*/
		$CustomerTickets = $this->CustomerTicket->query("SELECT customers.companyname,customers.firstname,customers.lastname, ct.id, ct.title, ct.description, ct.hours, ct.minutes, ct.comment FROM customer_tickets AS ct left JOIN customers AS customers ON (ct.customer_id = customers.id) WHERE ct.active = 1 ORDER BY ct.id desc;"); 
